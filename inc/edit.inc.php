...
//	check whether or not to include default translation subtables
	...
	$all_indices = array_keys($zz['fields']);
	asort($all_indices);
	...
	foreach (array_keys($zz['fields']) as $i) {
		...
		if (!empty($zz['fields'][$i]['field_name']) AND !empty($translationfields[$zz['fields'][$i]['field_name']])) {
			...
			$translationsubtable[$index+$k]['translate_field_name'] = $zz['fields'][$i]['field_name'];
...
//	Add, Update or Delete
	...
	$zz['fields_in_list'] = $zz['fields']; // conditions for list view will be set later
	...
	if (!empty($zz_conditions['values'])) {
		...
		foreach ($zz['conditional_fields'] as $condition => $definitions) {
			$template = $definitions['template_field'];
			if (!empty($definitions['remove_template'])) $remove_fields[$template] = true;
			if (empty($zz_conditions['values'][$condition])) continue;
			foreach ($zz_conditions['values'][$condition] as $placeholder_record) {
				...
				$new_fields[$template][$index] = $zz['fields'][$template];
		...
		foreach ($new_fields as $fieldindex => $fields_to_add) {
			$zz_fields = array_merge(array_slice($zz['fields'], 0, $all_indices[$fieldindex]), $fields_to_add, 
				array_slice($zz['fields'], $all_indices[$fieldindex]));
			// old PHP 4 support
...
//	Query Updated, Added or Editable Record
	...
	$extras = array();
	// keep some query arrays
	$keep_query_arrays = array('where', 'var');
	if ($zz['mode'] == 'add') $keep_query_arrays[] = 'add';
	foreach ($keep_query_arrays as $arr)
		if (!empty($_GET[$arr])) $extras[] = zz_get_to_array($_GET[$arr], $arr);
	// keep some query strings
	$keep_query_strings = array('order', 'group', 'q', 'scope', 'dir', 'referer', 'url');
	foreach ($keep_query_strings as $str)
		if (!empty($_GET[$str])) $extras[] = $str.'='.urlencode($_GET[$str]);
	...
	// first extraGET must be &amp; but not if there are no extras
	$zz_var['extraGET'] = ($extras ? '&amp;'.implode('&amp;', $extras) : '');
	...
	//
		...
		if ($result) 
			if (mysql_num_rows($result) == 1) {
				...
				$zz_error[]['msg'] = zz_text('Database error. This database has ambiguous values in ID field.');
	//
	} else
		$display_form = false;
