 ...
 */
function zz_validate($my, $db_table, $table_name, $k = 0) {
	global $zz_conf;
	global $zz_error;
	...
	//	remove entries which are for display only
		...
		//	...
			...
			if (($my['fields'][$f]['type'] == 'hidden' && !empty($my['fields'][$f]['upload_field'])) // type hidden, upload_field set
				...
				foreach ($possible_values AS $v) {
					if ($v == 'md5' AND !empty($my['images'][$g][0])) {
						if (!empty($my['images'][$g][0]['modified']['tmp_name']))
							$myval = md5_file($my['images'][$g][0]['modified']['tmp_name']);
						elseif (!empty($my['images'][$g][0]['upload']['tmp_name']))
							$myval = md5_file($my['images'][$g][0]['upload']['tmp_name']);
					} elseif ($v == 'md5_source_file' AND !empty($my['images'][$g][0]['upload']['tmp_name'])) {
						$myval = md5_file($my['images'][$g][0]['upload']['tmp_name']);
					} elseif ($v == 'sha1' AND !empty($my['images'][$g][0])) {
						if (!empty($my['images'][$g][0]['modified']['tmp_name']))
							$myval = sha1_file($my['images'][$g][0]['modified']['tmp_name']);
						elseif (!empty($my['images'][$g][0]['upload']['tmp_name']))
							$myval = sha1_file($my['images'][$g][0]['upload']['tmp_name']);
					} elseif ($v == 'sha1_source_file' AND !empty($my['images'][$g][0]['upload']['tmp_name'])) {
						$myval = sha1_file($my['images'][$g][0]['upload']['tmp_name']);
					} else {
		...
		//	first check for backwards compatibilty - old edit.inc does not include validation
			if (!$zz_conf['do_validation']) continue;
		...
		//	check against forbidden strings
			if (!empty($my['fields'][$f]['validate'])
				AND !empty($my['POST'][$my['fields'][$f]['field_name']])) {
				if ($msg = zz_check_validate($my['POST'][$my['fields'][$f]['field_name']], $my['fields'][$f]['validate'])) {
					$my['validation'] = false;
					$my['fields'][$f]['check_validation'] = false;
					$my['fields'][$f]['validation_error'] = $msg;
				}
			}
		}
	}
