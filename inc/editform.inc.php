	...
	if ($display) {
		...
		if ($my['mode'] && $my['mode'] != 'review') {
			...
			if ($zz_conf['list']) {
				...
				if ($zz_conf['edit']) {
					$output.= '<a href="'.$zz_conf['url_self'].'?mode=edit&amp;id='.$my_tab[0][0]['id']['value'].$my['extraGET'].'">'.$text['edit'].'</a>';
					if ($zz_conf['delete']) $output.= ' | <a href="'.$zz_conf['url_self'].'?mode=delete&amp;id='.$my_tab[0][0]['id']['value'].$my['extraGET'].'">'.$text['delete'].'</a>';
				}
				$output.= '</td></tr>'."\n";
			...
			if ($field['type'] == 'id') {
				...
				if (isset($field['number_type']) AND $field['number_type'] == 'latitude' || $field['number_type'] == 'longitude') {
					...
					if ($display == 'form') {
						...
						$outputf.= geo_editform($field['f_field_name'].'['.substr($field['number_type'],0,3), $var);
						$outputf.= ' <br> ';
					} elseif ($var) {
					...
					if ($my['record']) {
						if ($display == 'form') $outputf.= 'value="';
						if(!is_array($my['record'][$field['field_name']])) // this would happen if record is not validated
					...
					if ($display == 'form') {
						...
						if (isset($field['show_id']) && $field['show_id'] == 1) $outputf.= 'id="'.$field['f_field_name'].'"';
