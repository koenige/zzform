...
function zz_foldercheck($zz_tab) {
	...
	foreach ($zz_tab[0]['folder'] as $folder) {
		$path = zz_path_file2($folder, $zz_tab);
		$old_path = zz_path_file2($folder, $zz_tab, 'old');
	...
	// 	walk through all fields by type
		...
		case 'captcha':
			...
			if (!$my_rec['POST'][$field_name]) {
				$my_rec['fields'][$f]['check_validation'] = false;
				$my_rec['validation'] = false;
			} elseif (!zz_captcha_code($my_rec['POST'][$field_name])) {
				$my_rec['fields'][$f]['check_validation'] = false;
				$my_rec['validation'] = false;
			} else {
		...
		foreach ($tables as $table => $ids) {
			...
			foreach ($zz['fields'] as $no => $field) {
				...
				foreach ($field['image'] as $image) {
					foreach ($data as $line) {
						$path = zz_path_file2($image['path'], $line, 'line');
						if (!$path) continue;
