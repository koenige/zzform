...
/**
 * zzform
 * Database functions (MySQL-specific)
 *
 * Part of »Zugzwang Project«
 * https://www.zugzwang.org/modules/zzform
 *
 * Contents:
 * - MySQL logging functions: zz_db_log*()
 * - MySQL query operations: zz_db_fetch(), zz_db_change()
...
function zz_db_log($sql, $user = '', $record_id = false) {
	...
	if (!strstr($logging_table, '.')) {
		...
		$target = zz_db_table($check_sql[0]);
...
function zz_db_log_hook($sql) {
	wrap_include('sql', 'zzform'); // if used outside zzform
	static $hook = [];
