	...
	if ($mode && !in_array($mode, ['review', 'show'])) {
		...
		if ($zz_conf_record['edit']) {
			...
			foreach ($zz_tab[0][0]['fields'] as $field) {
				if (empty($field['link_record'])) continue;
				if (empty($field['link'])) continue;
...
// #1.3 SELECT has one or several results, let user select something
	...
	$selected_record = zz_field_select_get_record($field, $record);
	$selected_record = zz_translate($field, $selected_record);

	// 1.3.1: no form display = no selection, just display the values in the record
	if ($display !== 'form') {
		if (!$selected_record) return zz_return('');
		return zz_return(zz_field_select_value($selected_record, $field));
	}

	// ok, we display something!
	// re-index lines by id_field_name if it makes sense
	$too_many_records = $lines['too_many_records'] ?? false;
	if ($too_many_records) {
		$count_rows = $lines[0];
	} else {
		...
		if (!empty($field['show_hierarchy'])) {
			...
			if (!count($lines)) {
				if (!$selected_record) return zz_return(zz_field_select_empty_hierarchy($field));
				// re-edit record, something was posted, ignore hierarchy because 
				// there's only one record coming back
				else $lines = [$selected_record];
			}
		}
		// hierarchy subtree might change the amount of lines
...
/**
 * select with hierarchy has no data
 *
 * @param array $field
 * @return string
 */
...
function zz_field_select_sql_too_long($field, $record, $selected_record) {
	...
	if ($selected_record) {
		...
		$element['value'] = zz_field_select_value($selected_record, $field).' ';
	} else {
...
// #2a SELECT with set_folder
	if (!is_dir($field['set_folder']))
		wrap_error([
			'`%s` is not a folder. Check `["set_folder"]` definition.',
			['values' => [$field['set_folder']]]
		], E_USER_ERROR);
	$files = [];
		...
		if (!empty($field['enum_translated'])) {
			...
			foreach ($field['enum_translated'] as $translations) {
				if (!array_key_exists($key, $translations)) {
					wrap_error(['Mismatch of no. of enum translations in field “%s”', ['values' => [$field['title']]]]);
					continue;
				}
	...
	if (in_array($mode, ['add', 'edit', 'revise']) && $field['type'] === 'upload_image') {
		...
		foreach ($uploads as $imagekey => $image) {
			...
			if (empty($field['dont_show_file_link'])
				AND ((!isset($image['path_web']) AND !isset($field['path_web'])) OR zz_path_file($image['path'], $record_saved ?? $record))
				AND $data[$i]['link'] = zz_path_link($image['path_web'] ?? $field['path_web'] ?? $image['path'], $record_saved ?? $record)) {
		...
		$i = 0;
		foreach ($uploads as $imagekey => $image) {
			if ((isset($image['path_web']) OR isset($field['path_web'])) AND !zz_path_file($image['path'], $record)) continue;
			$link = zz_path_link($image['path_web'] ?? $field['path_web'] ?? $image['path'], $record);
			if (!$link) continue;
			$data[$i]['title'] = $image['title'] ?? '';
			$data[$i]['link'] = $link;
			$i++;
...
function zz_record_dependent_js($field, $dependent_field, $my_fields, $field_no) {
	...
	if ($my_fields[$field_no]['type'] === 'subtable') {
		if (!empty($dependent_field['subtable_field_name'])) {
			$dependent_field_name = $my_fields[$field_no]['table_name'].'[0]['.$dependent_field['subtable_field_name'].']';
		} else {
